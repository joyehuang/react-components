...
function CylindricalTextReveal({
  lines = DEFAULT_LINES,
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const linesToAnimate = lineRefs.current.filter((line): line is HTMLHeadingElement => line !== null)
      ...
      const renderCylinder = (progress: number) => {
        ...
        const rollProgress = clamp((progress - revealStart) / (1 - revealStart), 0, 1)
        const revealProgress = clamp((progress - revealStart) / Math.max(revealEnd - revealStart, 0.001), 0, 1)
        const exitProgress = clamp((progress - 0.86) / 0.14, 0, 1)
        const rollAngle = gsap.utils.interpolate(rollStartAngle, rollEndAngle, rollProgress)
    ...
    return () => ctx.revert()
  }, [angleStep, radius, revealEnd, revealStart, rollEndAngle, rollStartAngle, scrub])

  const sectionClassName = className ? `ctr ${className}` : 'ctr'
  ...
  return (
    <section ref={sectionRef} className={sectionClassName} style={sectionStyle} aria-label="Cylindrical text reveal">
      <div className="ctr__sticky" style={stickyStyle}>
        {showHint ? (
          <p className="ctr__hint">
            {hintText}
        ...
        <div className="ctr__stage" style={stageStyle}>
