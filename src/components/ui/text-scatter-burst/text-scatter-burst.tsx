...
function getBoundedDistance(
  ...
  const positiveLimits = limits.filter(
...
export function TextScatterBurst({
  text = "Bounce Back.",
  className,
  stageClassName,
  minHeight = "72vh",
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (let i = 0; i < letters.length; i += 1) {
        ...
        const isNear =
          pointer.active && !item.isSpace && triggerDist < triggerRadius;
        ...
        if (isNear && !item.isNear) {
          ...
          const randomDrift = normalize(
            randomUnit(i + 1.2) - 0.5,
            randomUnit(i + 4.6) - 0.5,
          );
        ...
        const rotate = clamp(
          item.angle * item.explode + item.vx * 2.3,
          -58,
          58,
        );
        ...
        node.style.filter = item.isSpace
          ? "none"
          : `blur(${blur.toFixed(2)}px)`;
        node.style.opacity = item.isSpace
          ? `${(1 - item.explode).toFixed(3)}`
          : "1";
      }
  ...
  return (
    <section
      className={cn(
        "text-scatter-burst min-h-[var(--tsb-min-height,72vh)] grid place-items-center bg-black",
        className,
      )}
    ...
    >
      <div
        ref={stageRef}
        className={cn(
          "text-scatter-burst__stage w-[min(94vw,var(--tsb-max-width,1020px))] min-h-[72vh] flex items-center justify-center flex-nowrap gap-[clamp(3px,0.5vw,8px)] select-none cursor-default overflow-hidden",
          stageClassName,
        )}
        style={stageStyle}
      >
        {chars.map((char, index) => (
          <span
            ...
            className={cn(
              "text-scatter-burst__char inline-block",
              char === " " && "is-space",
            )}
          >
            {char === " " ? "\u00A0" : char}
