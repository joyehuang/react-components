...
export type NeonNetworkColorOverrides = {
  lineBase: string;
  lineGlow: string;
  lineCore: string;
  nodeDot: string;
...
export function NeonNetwork({
  ...
  return (
    <div
      className={cn(
        "neon-network-demo w-[min(var(--nn-max-width,1024px),94vw)] mx-auto grid gap-4",
        className,
      )}
      style={cssVars}
    >
      <section
        className={cn(
          "neon-network relative w-full aspect-[2/1] mx-auto overflow-hidden isolate",
      ...
      >
        <svg
          className="neon-network__lines absolute inset-0 w-full h-full pointer-events-none"
        ...
        >
          ...
          {connections.map((connection) => (
            <g
              key={`${connection.id}-animated-${runId}`}
              className="connection-animated"
            >
              <path
                className="connection connection--glow"
                d={connection.d}
                pathLength={100}
              />
              <path
                className="connection connection--core"
                d={connection.d}
                pathLength={100}
              />
            </g>
        ...
        {nodes.map((node) => (
          <span
            key={node.id}
            className={cn(
              "neon-network__node absolute",
              node.highlighted && "is-highlighted",
            )}
            style={
              {
                left: `${node.x}%`,
        ...
        <div
          className="neon-network__core absolute left-1/2 top-1/2 grid place-items-center"
          aria-hidden="true"
        >
          <span className="neon-network__core-glow" />
          <svg
            className="neon-network__logo w-[clamp(44px,5.4vw,58px)] aspect-square"
            viewBox="0 0 64 64"
          >
      ...
      {showControls ? (
        <div className="neon-network__controls flex justify-center gap-3">
          <button
