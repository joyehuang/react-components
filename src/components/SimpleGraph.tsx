...
type SimpleGraphColorOverrides = {
  background: string
  panelBorder: string
  grid: string
  line: string
...
const clampDuration = (value: number): number => Math.max(200, Math.round(value))
...
function SimpleGraph({
  ...
  const chartPoints = useMemo(() => {
    if (points.length === 0) {
      return [] as Array<{ x: number; y: number; value: number; label: string; displayValue?: string }>
    }
    ...
    const usableWidth = VIEWBOX_WIDTH - CHART_PADDING.left - CHART_PADDING.right
    const usableHeight = VIEWBOX_HEIGHT - CHART_PADDING.top - CHART_PADDING.bottom
  ...
  return (
    <section className={`simple-graph ${className ?? ''}`} style={cssVars} aria-label="Simple animated graph">
      <div className="simple-graph__panel" onPointerLeave={() => setActiveIndex(null)}>
        <div className="simple-graph__canvas">
          <svg className="simple-graph__svg" viewBox={`0 0 ${VIEWBOX_WIDTH} ${VIEWBOX_HEIGHT}`} preserveAspectRatio="xMidYMid meet">
            <defs>
              <linearGradient id="simple-graph-area" x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor={mergedColors.line} stopOpacity="0.5" />
                <stop offset="100%" stopColor={mergedColors.line} stopOpacity="0" />
              </linearGradient>
            </defs>

            {showGrid ? (
              <g className="simple-graph__grid">
                {Array.from({ length: Math.max(gridLines, 2) }).map((_, index, arr) => {
                  const ratio = index / (arr.length - 1)
                  const y = CHART_PADDING.top + ratio * (VIEWBOX_HEIGHT - CHART_PADDING.top - CHART_PADDING.bottom)

                  return (
                    <line key={`grid-${index}`} x1={CHART_PADDING.left} x2={VIEWBOX_WIDTH - CHART_PADDING.right} y1={y} y2={y} />
                  )
                })}
              </g>
            ) : null}
            ...
            {chartPoints.map((point, index) => {
              ...
              return (
                ...
                >
                  <circle className="simple-graph__point-hit" cx={point.x} cy={point.y} r={6.5} tabIndex={0} />
                  <circle
        ...
        {activePoint ? (
          <div
            className="simple-graph__tooltip"
            style={
              {
            ...
            aria-live="polite"
          >
            <p className={`simple-graph__tooltip-value ${isNegative ? 'is-negative' : 'is-positive'}`}>
              {trendArrow} {trendText}
            </p>
            <p className="simple-graph__tooltip-label">{activePoint.label}</p>
          </div>
        ) : null}
      </div>
    </section>
