...
export type CreditCardProps = Omit<HTMLAttributes<HTMLDivElement>, 'color'> & {
  cardNumber?: string
  cardholder?: string
  expires?: string
  cvv?: string
...
const toCssDimension = (value: number | string) => (typeof value === 'number' ? `${value}px` : value)

const joinClassNames = (...parts: Array<string | undefined | false>) => parts.filter(Boolean).join(' ')
...
export const CreditCard = forwardRef<HTMLDivElement, CreditCardProps>(function CreditCard(
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      current.opacity += (target.opacity - current.opacity) * (pointerActiveRef.current ? 0.18 : 0.08)
      writeStyles()
  ...
  return (
    <div
      ref={ref}
      {...rest}
      className={joinClassNames('cc3d-root', className)}
      style={{ ...cssVars, ...(style as CSSProperties | undefined) }}
    >
      <div className="cc3d-stage" ref={stageRef}>
        <article
          className={joinClassNames('cc3d-card', isFlipped && 'is-flipped', clickToFlip && 'is-clickable')}
          ref={cardRef}
          ...
          aria-label={clickToFlip ? (isFlipped ? 'Flip card to front side' : 'Flip card to back side') : undefined}
